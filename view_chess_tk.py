...
from tkinter import *
from tkinter import ttk, Label, LabelFrame, Entry
import sqlite3
...
class ChessLstPlayerFrame(ttk.Frame):
    def __init__(self, master, title="Player"):
        ...
        #player_fr.grid(row=0, column=0, padx=8,pady=8, sticky='wn')
        ...
        #self.birthdatefield = Entry(player_fr, textvariable=self.birthdate)
        #self.birthdatefield = Entry(player_fr, textvariable=birthdate_valid, command = cal1.selection_get())
        ...
        # BOUTONS
        # ++++++++++++++++++++++++++++++++++++++
        """ttk.Button(player_fr, text='Ajouter à la liste', command=self.create_record).grid(
            row=0, column=0, sticky='nw', padx=210, pady=135
            )"""
        ttk.Button(player_fr, text='Ajouter à la liste', command=self.create_player).grid(
            row=0, column=0, sticky='nw', padx=210, pady=135)
        # birthdate_valid = ttk.Button(player_fr, text="date validation", command=cal1.selection_get()).grid(row=0, column=0, padx=20, pady=20, sticky='e')                                                                             
        ...
        # FORMULAIRE LISTING 
        # ++++++++++++++++++++++++++++++++++++++
        my_columns = ("Id", "Name", "Firstname", "Gender", "Birthdate", "Rank",)
    ...
    def delete_record(self):
        ...
        # self.msg["text"] = "Le classment de %s est supprimé " %query
        self.view_records()
...
class TournamentFrame(ttk.Frame):
    def __init__(self, master):
        ...
        #  liste
        # ===============================================================
        #ttk.Button(chess_fr, text='Ajouter à la liste', command=self.create_record).grid(
    ...
    def view_records(self):
        ...
        conn = sqlite3.connect('phonebook.db')
        c = conn.cursor()
        #list = c.execute("SELECT * FROM Player ORDER BY name desc")
...
class Turn(Frame):
    def __init__(self, master):
        ...
        ttk.Button(fr, text= 'Ajouter à la liste', command=self.create_record).grid(row=5, column=2, sticky=E,padx=5, pady=2)
...
class ReportFrame(ttk.Frame):
    def __init__(self, master):
        ...
        self.match_tree = ttk.Treeview(fr,height=6, columns=("start_date", "end_date", "Player 1", "Player 2", "Results"))
